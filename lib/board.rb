...
class Board
  ...
  def place_ship(ship, location, direction)
    ...
    proposed_squares = calculate_squares(ship, location, direction)

    if valid_placement? proposed_squares, location
      ships.store(ship, proposed_squares)
    else
  ...
  def valid_placement?(proposed_squares, location)
    if location.to_s.bytesize > 2
      return false
    end
    
    outside_board?(proposed_squares)
  end

  def calculate_squares(ship, location, direction)
